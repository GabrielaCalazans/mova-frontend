...
function CarrosScreen() {
  ...
  // Filtragem local por texto (marca, modelo, placa) e por tipo de carro
  const veiculosFiltrados = veiculos.filter((v) => {
    // Filtro por texto
    if (textoBusca.trim()) {
      const termo = textoBusca.toLowerCase();
      if (!resolveVeiculoTextoBusca(v).includes(termo)) return false;
    }
    ...
    if (tipoFiltro === "economico") {
      ...
      if (v.cambio?.toLowerCase() !== "automatico" || v.eletrico || v.adaptado)
        return false;
      ...
      >
        {[
          ...
          <button
            ...
            type="button"
            onClick={() =>
              setTipoFiltro((prev) => (prev === tipo.id ? null : tipo.id))
            }
            style={{
              display: "flex",
              ...
              padding: "0.6rem 1rem",
              border:
                tipoFiltro === tipo.id ? "2px solid #003366" : "1.5px solid #ccc",
              borderRadius: "12px",
              background: tipoFiltro === tipo.id ? "#e8f0fe" : "#fff",
              cursor: "pointer",
              fontSize: "0.8rem",
              fontWeight: 600,
